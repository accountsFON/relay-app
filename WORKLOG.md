...
- [ ] **(config, Julio) Set `NEXT_PUBLIC_APP_URL` in prod** to the friendly domain so review links don't depend on the Vercel alias fallback (see PR #268). Vercel dashboard setting, not a code change; the code fallback stays as a safety net.
- [ ] **(note, image replace) IG/FB posts call `useRouter` (via the shared image-replace hook)** — any test rendering a real InstagramFeedPost/FacebookPost must mock the router / hook. Existing tests handled; keep in mind for new ones.

Cleared 2026-08-07: Bell "Post N" copy (#415); `LIVE_PIPELINE_STEPS` client-import boundary, moved to `@/lib/relay-transitions` (#413); DesignerFlag duplicate-flag race, closed in code with a per-target advisory-lock transaction instead of a null-aware unique index Prisma can't express (#417); stale `reviewSessionId` JSDoc (#413); designer-gate archived-batch skip test, via an extracted `designerGateApplies` predicate (#414); `startIfUnseen` already-active guard-branch test (#414); consolidated `DesignerRevisionUpload` + `MediaUpload` onto `useReplacePostImage` (#416).
...
- [x] **2026-08-06 — Tooltip coverage: deferred slices (Edit copy link + profile edit pencils)** (PR #395)
  ...
  (2) The client-profile field edit pencils now show a hover hint: the shared FieldHeader pencil (`Edit {label}`,
  covers all field types) and the FocusField pencil. tsc + 2661 unit tests + `next build` clean; the one eslint
  error in client-profile-view.tsx is pre-existing (line 187 setState-in-effect), not from this change. This
  completes the full-journey tooltip rollout.
...
- [x] **2026-08-05 — Confirm before discarding a dirty inline caption edit** (PR #385)
  ...
  silently. Single parent (`review-post-card`) covers both IG + FB. Scope deliberately narrow — no other cancels touched.
  From Rebecca's Sept 2026 QA test. tsc + 2631 tests + `next build` clean. Deploy verification pending merge.
...
- [x] **2026-08-04 — Brand-new-signup landed on a dead /welcome (no buttons, no tour, needed a refresh)** (PR #376)
  ...
  that landing via the server-action navigation, which never attached client interactivity. A plain refresh (a full
  document load) fixed it, which proves a fresh load hydrates fine. Fix: the action RETURNS the destination and the
  client (`onboarding-form.tsx`) does `window.location.assign` (full-document navigation) to it. Confirmed live: an
  invited Designer signup (julygerman) hit the dead page pre-deploy exactly as diagnosed. tsc + 2624 tests + `next build`
  clean.
...
- [x] **2026-07-22 — Remove dead component files + unused dependencies** (PR #362, `0d3425f`)
  ...
  `review-attention-card`, `review-pinned-post-client`, `review-submitted-screen`, `send-link-button`,
  `client-list-item`, `preview-submit-button`, and unused `ui/` primitives `action-bar`, `info-tooltip`,
  `separator`. Removed 14 unused deps: `@aws-sdk/client-s3`, `@aws-sdk/s3-request-presigner`,
  `@neondatabase/serverless`, `@prisma/adapter-neon` (db uses `@prisma/adapter-pg` + `pg`), the 6 dead
  `@radix-ui/react-*` (Relay is on base-ui), `@tanstack/react-query`(+devtools), `next-safe-action`,
  `stripe` (only DB column names). KEPT 3 knip flagged as unused but are actually used via entry points
  knip can't trace: `@clerk/testing` (`e2e/auth.setup.ts`), `@types/diff` (`src/lib/text-diff.ts`),
  `@types/testing-library__jest-dom` (`tests/setup.ts`). Verified nothing broke: tsc + 2589 tests +
  `next build` all clean. Lockfile pruned substantially. No migration, no jobs.
