...
- [ ] **(feature) Review resolve-checklist — slice 4** — slices 1 (PR #297), 2 (PR #298), 3 (PR #299) shipped. Remaining: **slice 4** = client magic-link changes navigation (filter + item stepper, navigation-only, no resolve). Spec: `2026-07-01-review-changes-checklist-design.md`.
- [ ] **(follow-up, slice 3 UX polish)** non-blocking from the whole-branch review: (1) `ChangesNavigator` cursor persists across server refresh (stepper can point at a now-resolved item; harmless, Next still finds the next unresolved); (2) with "Changes only" on, the stepper walks the full unresolved set incl. filtered-out posts (canvas scroll lands, rail row not shown); (3) dead `|| post.addressed` in `review-feedback-rail` `capDone`; (4) `onScrollToAnchor` optional with a silent no-op — make it required or warn.
