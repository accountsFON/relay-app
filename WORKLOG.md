...
- [ ] **(follow-up, tenant safety) Verify org-scoping on the `threads.ts` client/AM thread actions** — `createThreadAction` / `addCommentAction` / `resolveThreadAction` use `resolveActor()` (serving BOTH Clerk AMs and token-scoped magic-link reviewers), a different auth model than the `ctx.organizationDbId` gate. Not a confirmed gap — the magic-link path is token-bound and AMs resolve through the internal roster — but the cross-tenant boundary for an authenticated AM passing a foreign `postId`/`threadId` deserves a dedicated trace. (surfaced while hardening `tickChecklistItemAction`)
